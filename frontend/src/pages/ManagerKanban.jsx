...
export default function ManagerKanban() {
  ...
  const assign = async (jobId) => {
    ...
    try {
      const res = await api.patch(`/jobcards/${jobId}/assign`, {
        technicianId: techId,
      });
                ...
                >
                  <div style={{ flex: 1, marginRight: 12 }}>
                    <div>
                      ...
                      <span className="small">{c.customerName}</span>
                    </div>
                    <div className="small" style={{ marginTop: 6 }}>
                      {c.reportedIssue}
                    </div>
                    <div className="small" style={{ marginTop: 6 }}>
                      Assigned: {c.assignedTo?.name || <em>—</em>}
                    </div>
                  </div>
                  ...
                  {!isAssigned ? (
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button className="btn" onClick={() => assign(c._id)}>
                          Assign
                        </button>
                        <button
                          className="btn ghost"
