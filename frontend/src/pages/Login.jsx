...
export default function Login(){
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f5f7fb',padding:20}}>
      <div className="card" style={{width:420,padding:24,borderRadius:12}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          <div style={{fontSize:20,fontWeight:700,color:'#2563eb'}}>JobCard — Service</div>
          <div className="small">Sign in</div>
        </div>

        {error && <div className="card small" style={{background:'#fee2e2',color:'#991b1b',marginBottom:12}}>{error}</div>}

        <form onSubmit={submit}>
          <label className="small" style={{marginBottom:6,display:'block'}}>Email</label>
          <input className="input" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="you@company.com" />

          <label className="small" style={{marginTop:10,marginBottom:6,display:'block'}}>Password</label>
