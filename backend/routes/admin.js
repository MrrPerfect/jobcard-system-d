...
router.get(
  ...
  requireRole("manager"),
  async (req, res) => {
    const pending = await User.find({ approved: false }).select(
  ...
  async (req, res) => {
    try {
      ...
      if (req.user._id.toString() === targetId && role !== "manager") {
        return res
          .status(400)
          .json({ message: "Managers cannot demote themselves" });
      }
      ...
      if (target.role === "manager" && role !== "manager") {
        ...
        if (managerCount <= 1) {
          return res
            ...
            .json({
              message:
                "Cannot remove the last manager. Promote another user first.",
            });
        }
      }
